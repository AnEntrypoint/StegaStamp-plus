...
checkpoints = sorted([int(f.split('_')[2].split('.')[0]) for f in glob.glob('encoder_step_*.keras')])
RESUME_FROM = checkpoints[-1] if checkpoints else 0
if RESUME_FROM == 0:
    raise FileNotFoundError("No checkpoints found. Start training with train.py first.")
...
print(f"Loading models from checkpoint {RESUME_FROM}...", flush=True)
custom_objects = {'Encoder': Encoder, 'Decoder': Decoder}
encoder = keras.models.load_model(f'encoder_step_{RESUME_FROM}.keras', custom_objects=custom_objects)
decoder = keras.models.load_model(f'decoder_step_{RESUME_FROM}.keras', custom_objects=custom_objects)
print(f"✓ Models loaded from step {RESUME_FROM}", flush=True)
...
print(f"Resuming training from step {RESUME_FROM} to {NUM_STEPS}...", flush=True)
...
print(f"  Resume step: {RESUME_FROM}", flush=True)
...
for step in range(RESUME_FROM, NUM_STEPS):
...
elapsed = (time.time() - start_time) / 60
print(f"✓ Training complete in {elapsed:.1f}m", flush=True)
print(f"✓ Test accuracy: {accuracy:.2%}", flush=True)

encoder.save('encoder_final.keras')
decoder.save('decoder_final.keras')
print(f"✓ Final models saved", flush=True)
