...
SECRET_BITS, IMG_SIZE = 256, 128

img_in = keras.Input(shape=(IMG_SIZE, IMG_SIZE, 3))
x = keras.layers.Conv2D(32, 3, padding='same', activation='relu')(img_in)
x = keras.layers.Conv2D(32, 3, padding='same', activation='relu')(x)
x = keras.layers.Conv2D(3, 3, padding='same', activation='sigmoid')(x)
encoder = keras.Model(inputs=img_in, outputs=x)

img_in2 = keras.Input(shape=(IMG_SIZE, IMG_SIZE, 3))
x = keras.layers.Conv2D(64, 3, padding='same', activation='relu')(img_in2)
...
x = keras.layers.Dense(SECRET_BITS, activation='sigmoid')(x)
decoder = keras.Model(inputs=img_in2, outputs=x)
