...
print("Testing 100-bit with 10 steps...", flush=True)
NUM_STEPS = 140000
...
for step in range(10):
    ...
    print(f"[TEST] Step {step:2d} | Secret:{float(sec_loss):.4f} L2:{float(l2_loss):.4f} Total:{float(tot_loss):.4f} | ResidualMag:{res_mean:.6f} ResidualMax:{res_max:.6f} | L2scale:{float(l2_scale):.4f}", flush=True)

print("\n=== 100-bit ANALYSIS ===", flush=True)
print("If Secret loss is DECREASING from ~0.69: 100-bit works ✓", flush=True)
print("If Secret loss STAYS at ~0.69: Something is wrong ✗", flush=True)
