...
print("Testing FIXED 256-bit encoder...")
...
print(f"✓ Encoder residuals for zeros: mean={tf.reduce_mean(tf.abs(residual_zeros)).numpy():.6f}")
print(f"✓ Encoder residuals for ones:  mean={tf.reduce_mean(tf.abs(residual_ones)).numpy():.6f}")
print(f"✓ Residual difference (zeros vs ones): {tf.reduce_mean(tf.abs(residual_zeros - residual_ones)).numpy():.6f}")
...
print(f"\n✓ Decoder output for zeros secret: mean={decoded_zeros.mean():.4f}, std={decoded_zeros.std():.4f}")
...
print(f"\n✓ Secret loss for zeros: {secret_loss_zeros.numpy():.6f} (target: 0.0 for perfect)")
print(f"✓ Secret loss for ones:  {secret_loss_ones.numpy():.6f} (target: 0.0 for perfect)")
print(f"✓ Random baseline: 0.693147")

if secret_loss_zeros.numpy() < 0.69 or secret_loss_ones.numpy() < 0.69:
    print("\n✓✓✓ DECODER IS LEARNING! Loss below random baseline!")
else:
    print("\n❌ Decoder still not learning (loss at random baseline)")
