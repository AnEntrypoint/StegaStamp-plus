...
def lpips_loss_simple(x, y):
    diff = x - y
    lpips = tf.reduce_mean(tf.square(diff))
    return lpips

def train_step(images, secrets, secret_scale, l2_scale, lpips_scale):
    with tf.GradientTape(persistent=True) as tape:
        ...
        lpips = lpips_loss_simple(encoded, images)

        total_loss = secret_scale * secret_loss + l2_scale * l2_loss + lpips_scale * lpips
    ...
    return secret_loss, l2_loss, lpips, total_loss

def get_loss_scales(step, total_steps):
    secret_scale = tf.constant(1.5, dtype=tf.float32)
    l2_ramp = int(total_steps * 0.1)
    l2_scale = tf.constant(min(2.0, (step / l2_ramp) * 2.0) if step < l2_ramp else 2.0, dtype=tf.float32)
    lpips_ramp = int(total_steps * 0.15)
    lpips_scale = tf.constant(min(1.5, (step / lpips_ramp) * 1.5) if step < lpips_ramp else 1.5, dtype=tf.float32)
    return secret_scale, l2_scale, lpips_scale

print(f"Training 100-bit secrets with LPIPS for {NUM_STEPS} steps...", flush=True)
for step in range(NUM_STEPS):
    images, secrets = get_img_batch(BATCH_SIZE)
    images = tf.constant(images)
    secrets = tf.constant(secrets)

    secret_scale, l2_scale, lpips_scale = get_loss_scales(step, NUM_STEPS)
    sec_loss, l2_loss, lpips, tot_loss = train_step(images, secrets, secret_scale, l2_scale, lpips_scale)

    if (step + 1) % 500 == 0:
        print(f"Step {step+1}/{NUM_STEPS} Secret:{float(sec_loss):.4f} L2:{float(l2_loss):.4f} LPIPS:{float(lpips):.6f} Total:{float(tot_loss):.4f}", flush=True)

    if (step + 1) % 10000 == 0:
        encoder.save(f'encoder_100bit_lpips_step_{step+1}.keras')
        decoder.save(f'decoder_100bit_lpips_step_{step+1}.keras')
        print(f"✓ Checkpoint saved at step {step+1}", flush=True)

encoder.save('encoder_100bit_lpips_final.keras')
decoder.save('decoder_100bit_lpips_final.keras')
